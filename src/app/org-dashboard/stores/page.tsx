...
export default function StoreManagementPage() {
  const [prokers, setProkers] = useState<Proker[]>([
    { id: "PRK-001", nama: "Dies Natalis ke-30", deskripsi: "Kepanitiaan acara ulang tahun fakultas", status: "Aktif", totalProduk: 12 },
    { id: "PRK-002", nama: "LKMM Dasar 2026", deskripsi: "Latihan Kepemimpinan Manajemen Mahasiswa", status: "Aktif", totalProduk: 5 },
    { id: "PRK-003", nama: "Bakti Sosial Desa", deskripsi: "Program pengabdian masyarakat di desa mitra", status: "Aktif", totalProduk: 3 },
    { id: "PRK-004", nama: "Penyambutan Maba", deskripsi: "Kepanitiaan penerimaan mahasiswa baru", status: "Selesai", totalProduk: 15 },
  ]);
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (modalMode === "add") {
      ...
      setProkers(prokers.map(p => p.id === currentProker.id ? { ...p, ...currentProker } as Proker : p));
    }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Manajemen Sub-Toko (Proker)</h1>
          <p className="text-sm text-slate-500">Kelola pendaftaran, edit data, atau nonaktifkan akun panitia proker Anda.</p>
        </div>
        <button 
          ...
          >
            <div className="p-5 border-b border-slate-100 flex justify-between items-start">
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-base">{proker.nama}</h3>
                  <p className="text-xs text-slate-400 font-mono mt-0.5">{proker.id}</p>
                </div>
              </div>
            ...
            <div className="p-5 flex-1">
              <p className="text-sm text-slate-600 mb-4 line-clamp-2">{proker.deskripsi}</p>
              
              <div className="flex justify-between items-center mt-auto">
                <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${
                  proker.status === 'Aktif' ? 'bg-emerald-100 text-emerald-700' :
                  proker.status === 'Selesai' ? 'bg-slate-100 text-slate-600' :
            ...
            >
              <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h2 className="text-lg font-bold text-slate-900">
                  {modalMode === "add" ? "Daftarkan Proker Baru" : "Edit Profil Proker"}
                </h2>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Nama Program Kerja</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Status Operasional</label>
                  <select 
        ...
        {isDeleteModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm">
            ...
            >
              ...
              <h2 className="text-xl font-bold text-slate-900 mb-2">Hapus Proker?</h2>
              <p className="text-slate-500 text-sm mb-6">
                Apakah Anda yakin ingin menghapus <strong>{currentProker.nama}</strong>? Semua produk dan data pesanan yang terkait tidak dapat dikembalikan.
              </p>
              <div className="flex gap-3">
                <button 
