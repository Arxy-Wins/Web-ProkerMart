...
import { UserPlus, UserCog, Mail, ShieldAlert, Settings2, Minus, Plus, ShoppingBag, Lock } from "lucide-react";
...
export default function TeamPage() {
  ...
  const canAddMember = ["Ketua Proker", "Wakil Proker", "Sekretaris Proker", "Bendahara Proker", "Koor Penggalian Dana"].includes(currentUserRole);
  const canSetTarget = ["Ketua Proker", "Wakil Proker", "Sekretaris Proker", "Bendahara Proker", "Koor Penggalian Dana"].includes(currentUserRole);
  const canLogSales = ["Koor Penggalian Dana", "Anggota Penggalian Dana"].includes(currentUserRole);
  ...
  const [team, setTeam] = useState([
    { id: 1, name: "Agung Arda Swari", role: "Ketua Proker", email: "arda@mahasiswa.unud.ac.id", jualanMingguIni: 3 },
    { id: 2, name: "Nyoman Tryasti", role: "Koor Penggalian Dana", email: "tryasti@mahasiswa.unud.ac.id", jualanMingguIni: 2 },
    { id: 3, name: "Gede Oka Adyuta", role: "Anggota Penggalian Dana", email: "oka@mahasiswa.unud.ac.id", jualanMingguIni: 0 },
    { id: 4, name: "Dewa Pradnyana", role: "Anggota Penggalian Dana", email: "dewa@mahasiswa.unud.ac.id", jualanMingguIni: 1 },
  ]);

  const handleAddSales = (id: number) => {
    setTeam(team.map(m => m.id === id ? { ...m, jualanMingguIni: m.jualanMingguIni + 1 } : m));
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      ...
      <div className="bg-slate-900 text-white p-4 rounded-xl flex items-center justify-between shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-sm">Simulasi Hak Akses (RBAC)</span>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-blue-50 border border-blue-200 p-5 rounded-2xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-700 mt-1">Jabatan: <strong className="bg-blue-200 px-1 py-0.5 rounded">{currentUserRole}</strong></p>
                <ul className="text-xs text-blue-700 mt-2 list-disc pl-4 space-y-1">
                  ...
                  <li>Input Jualan: {canLogSales ? '✅ Diizinkan' : '❌ Ditolak'}</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 p-5 rounded-2xl shadow-sm">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6">
              <p className="text-xs font-semibold text-slate-500 mb-3 text-center">Wajib Jualan Per Anggota</p>
              <div className="flex items-center justify-between">
                ...
                <div className="text-center">
                  <span className="text-3xl font-black text-slate-900">{targetPerMinggu}</span>
                  <span className="text-xs text-slate-500 block">Kali/Minggu</span>
                </div>
                <button 
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm">
                ...
                <span className="font-bold text-slate-900">{team.length} Orang</span>
              </div>
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-600">Memenuhi Target</span>
                <span className="font-bold text-emerald-600">
                  {team.filter(m => m.jualanMingguIni >= targetPerMinggu).length} Orang
                </span>
              </div>
            </div>
          </div>
            ...
            return (
              ...
              >
                <div className={`absolute top-0 left-0 w-1 h-full ${
                  member.role.includes('Ketua') || member.role.includes('Wakil') || member.role.includes('Bendahara') || member.role.includes('Sekretaris') 
                    ? 'bg-purple-500' 
                    : member.role.includes('Koor') ? 'bg-primary-500' : 'bg-slate-300'
                }`}></div>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-medium text-primary-600">{member.role}</p>
                    </div>
                  </div>
                ...
                <div className="mt-2 bg-slate-50 p-3 rounded-xl border border-slate-100">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-bold text-slate-700">Progress Keliling</span>
                    ...
                    {[...Array(targetPerMinggu)].map((_, index) => {
                      ...
                      return (
                        <div key={index} className="flex-1">
                          <div className={`h-2.5 w-full rounded-sm ${isCompleted ? 'bg-emerald-500' : 'bg-slate-200'}`}></div>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  {canLogSales ? (
                    ...
                    <div className="text-center text-[10px] text-slate-400 bg-white border border-slate-100 py-1 rounded flex justify-center items-center gap-1">
                      <Lock className="w-3 h-3" /> Hanya Pengdan yang bisa rekap jualan
                    </div>
                  )}
                </div>
